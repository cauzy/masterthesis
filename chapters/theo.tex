...
``Digial Lock-in'' describes the principle that only original components are compatabile with a already buyed system. The manufactor of a device can decide wich add-ons for a product can be applied. This concept limits compatibility but enhances warrantie issues.
...
By submitting information about the environment in real time, things like error predicition can be established. In the non-IoT approach, this was only possible with high costs. With IoT, such predicition mechanisms can be deployed at low cost anywhere in the world.
...
On an industrial perspective, the Internet of Things opens the marked for smart, connected and automated systems that can be used in production facilites and manufactoring systems. Because of this, Germany introduced the term ``Industry 4.0'' together with an initiative of the German Federal Ministry of Education and Research.

IoT can affect the industry in three differnt ways:
\begin{enumerate}
