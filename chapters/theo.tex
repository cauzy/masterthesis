...
\end{figure}
blub bub blbu
The following enumerations explains the five layers used in an IoT environment in detail \cite{iotfleisch}.
...
This layer connects the physical thing and the sensors/actuators to the Internet. Within this layer the device gets accessible via the Internet by, for example, a radio module that is connected to the local wireless network.

\subsubsection{Layer 4: Analytics}
In this layer, the collected data from the sensors are collected and stored for further processing. Additional web services can be integrated to generate actions for the actuators or add additional information to the sensor data. In this layer and additional value is added to the 'stupid' thing.

\subsubsection{Layer 5: Digital service}
This layer combines all the previous layers and offers a functionality as a digital service. E.g. a mobile application with world-wide access to control the lightbulb.

\subsection{Comparision with 'Industrial Internet of Things'}

\subsection{Comparision with 'Industry 4.0'}

\subsection{Impact on this thesis}